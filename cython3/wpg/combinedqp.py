...
class NMPCGenerator(BaseGenerator):
    ...
    def __init__(self, N=16, T=0.2, T_step=1.6, fsm_state='D', fsm_sl=1):
        ...
        self.Hx     = np.zeros((1, 2*(N+nf)), dtype=float)
        self.Q_k_x  = np.zeros((N+nf, N+nf),  dtype=float)
        self.p_k_x  = np.zeros((N+nf,),       dtype=float)
        self.p_k_y  = np.zeros((N+nf,),       dtype=float)

        self.Hq     = np.zeros((1, 2*N), dtype=float)
        self.Q_k_qR = np.zeros((N, N),   dtype=float)
        self.Q_k_qL = np.zeros((N, N),   dtype=float)
        self.p_k_qR = np.zeros((N),      dtype=float)
        self.p_k_qL = np.zeros((N),      dtype=float)

        self.A_pos_x   = np.zeros((self.nc_pos, 2*(N+nf)), dtype=float)
        self.A_pos_q   = np.zeros((self.nc_pos, 2*N), dtype=float)
        self.ubA_pos = np.zeros((self.nc_pos,), dtype=float)
        self.lbA_pos = np.zeros((self.nc_pos,), dtype=float)

        self.A_ori   = np.zeros((self.nc_ori, 2*N), dtype=float)
        self.ubA_ori = np.zeros((self.nc_ori,),     dtype=float)
        self.lbA_ori = np.zeros((self.nc_ori,),     dtype=float)

        self.derv_Acop_map = np.zeros((self.nc_cop, self.N), dtype=float)
        self.derv_Afoot_map = np.zeros((self.nc_foot_position, self.N), dtype=float)
    ...
    def _calculate_derivatives(self):
        ...
        rotMat1 = np.array([
            ...
            [-np.sin(theta_vec[0]), np.cos(theta_vec[0])],
            [-np.cos(theta_vec[0]),-np.sin(theta_vec[0])]
        ])
        rotMat2 = np.array([
            # old
            ...
            # new
            [-np.sin(theta_vec[1]), np.cos(theta_vec[1])],
            [-np.cos(theta_vec[1]),-np.sin(theta_vec[1])]
        ])
        nf = self.nf
